...
const BILIBILI_COOKIE_FILE = process.env.BILIBILI_COOKIE_FILE || path.join(__dirname, '.bili-cookie');
const UPDATE_WORK_DIR = process.env.AR1S_UPDATE_DIR || path.join(__dirname, 'updates');
const UPDATE_DOWNLOAD_DIR = process.env.AR1S_UPDATE_DOWNLOAD_DIR || path.join(UPDATE_WORK_DIR, 'downloads');
const UPDATE_PATCH_BACKUP_DIR = process.env.AR1S_PATCH_BACKUP_DIR || path.join(UPDATE_WORK_DIR, 'backups', 'patches');
const BEATMAP_CACHE_DIR = process.env.AR1S_BEAT_CACHE_DIR || 'D:\\AR1SCache\\beatmaps';
const APP_PACKAGE = readPackageInfo();
const APP_VERSION = process.env.AR1S_VERSION || APP_PACKAGE.version || '0.9.11';
...
let biliCookie = '';
try { if (fs.existsSync(BILIBILI_COOKIE_FILE)) biliCookie = fs.readFileSync(BILIBILI_COOKIE_FILE, 'utf8').trim(); }
catch (e) { biliCookie = ''; }
function saveBiliCookie(c) {
  biliCookie = normalizeCookieHeader(c) || rawCookieFallback(c);
  try { fs.writeFileSync(BILIBILI_COOKIE_FILE, biliCookie); } catch (e) {}
}
function biliCookieObject() {
  return parseCookieString(biliCookie);
}
function biliCookieUid(obj) {
  obj = obj || biliCookieObject();
  return obj.DedeUserID || obj.DedeUserID__ckMd5 || obj.uid || '';
}
function biliCookieSessdata(obj) {
  obj = obj || biliCookieObject();
  return obj.SESSDATA || obj.sessdata || '';
}
function biliCookieNickname(obj) {
  obj = obj || biliCookieObject();
  return decodeURIComponent(obj.DedeUserID__ckMd5 || '') || obj.uname || obj.nickname || '';
}
function biliCookieAvatar(obj, uid) {
  obj = obj || biliCookieObject();
  const face = obj.face || obj.avatar || '';
  if (face) return decodeURIComponent(face);
  return '';
}
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.ar1s && pkg.ar1s.update) || {};
  const repoHint = process.env.AR1S_UPDATE_REPOSITORY
  ...
  const owner = process.env.AR1S_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.AR1S_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.AR1S_UPDATE_MANIFEST
      || process.env.AR1S_UPDATE_MANIFEST_URL
      || process.env.AR1S_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.AR1S_UPDATE_MIRRORS || process.env.AR1S_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `AR1S-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `AR1S-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('AR1S v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `AR1S/${APP_VERSION}` },
    });
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `AR1S-${latestVersion}-Setup.exe`;
  ...
  try {
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('AR1S v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `AR1S-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `AR1S-${version || APP_VERSION}.exe`;
}
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'ar1s-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
function audioProxyHeadersFor(audioUrl, range) {
  ...
  try {
    ...
    } else if (host.includes('bilibili.com') || host.includes('bilivideo.com') || host.includes('hdslb.com')) {
...
async function biliGetJSON(targetUrl, params, opts) {
  opts = opts || {};
  const u = new URL(targetUrl);
  Object.keys(params || {}).forEach(k => {
    if (params[k] != null) u.searchParams.set(k, String(params[k]));
  });
  const headers = { ...BILIBILI_HEADERS, ...(opts.headers || {}) };
  if (opts.cookie !== false && biliCookie) headers.Cookie = biliCookie;
  const text = await requestText(u.toString(), { headers });
  return parseJSONText(text);
}
async function getBiliLoginInfo() {
  const uid = biliCookieUid();
  const sessdata = biliCookieSessdata();
  if (!uid || !sessdata) return { provider: 'bilibili', loggedIn: false, hasCookie: !!biliCookie };
  try {
    const result = await biliGetJSON('https://api.bilibili.com/x/web-interface/nav', {}, { cookie: true });
    if (result && result.code === 0 && result.data) {
      const d = result.data;
      return {
        provider: 'bilibili',
        loggedIn: d.isLogin === true,
        userId: String(d.mid || uid),
        nickname: d.uname || biliCookieNickname() || 'B站用户',
        avatar: d.face || biliCookieAvatar() || '',
        level: d.level_info && d.level_info.current_level || 0,
        hasCookie: !!biliCookie,
      };
    }
    if (result && result.code === -101) {
      saveBiliCookie('');
      return { provider: 'bilibili', loggedIn: false, hasCookie: false };
    }
    return {
      provider: 'bilibili',
      loggedIn: !!(uid && sessdata),
      userId: uid,
      nickname: biliCookieNickname() || 'B站用户',
      avatar: biliCookieAvatar(),
      hasCookie: !!biliCookie,
      profileUnavailable: true,
    };
  } catch (e) {
    console.warn('[BiliLogin] check failed:', e.message);
    return {
      provider: 'bilibili',
      loggedIn: !!(uid && sessdata),
      userId: uid,
...
function mapBiliVideoResult(v) {
  ...
  return {
    ...
    aid: v.aid || v.id || 0,
    name: v.title || v.name || '',
    artist: v.author || (v.owner && v.owner.name) || '',
    artistMid: v.mid || (v.owner && v.owner.mid) || 0,
    artistAvatar: (v.owner && v.owner.face) || v.authorAvatar || '',
    cover: v.pic || v.cover || '',
    duration: (v.duration || 0) * 1000,
    playCount: v.play || v.view || 0,
    danmakuCount: v.danmaku || v.danmu || 0,
    description: v.description || v.desc || '',
    pubdate: v.pubdate || v.created || 0,
    tag: v.tag || v.typename || '',
  };
}
async function handleBiliSearch(keywords, limit) {
  const kw = String(keywords || '').trim();
  if (!kw) return [];
  console.log('[BiliSearch]', kw, 'limit:', limit);
  try {
    const result = await biliGetJSON('https://api.bilibili.com/x/web-interface/search/type', {
      search_type: 'video',
      keyword: kw,
      page: 1,
      order: 'totalrank',
    ...
    if (result && result.code === 0 && result.data && result.data.result) {
      const videos = (result.data.result || []).slice(0, limit);
      return videos.map(mapBiliVideoResult);
    }
    return [];
  } catch (e) {
    console.error('[BiliSearch] error:', e.message);
    return [];
  }
}
async function handleBiliVideoUrl(bvid) {
  if (!bvid) return { provider: 'bilibili', url: '', audioUrl: '', playable: false, error: 'MISSING_BVID' };
  console.log('[BiliVideoUrl] bvid:', bvid);
  try {
    // 先获取视频信息得到 cid
    const info = await biliGetJSON('https://api.bilibili.com/x/web-interface/view', { bvid }, { cookie: true });
    if (!info || info.code !== 0 || !info.data) {
      return { provider: 'bilibili', url: '', audioUrl: '', playable: false, error: 'VIDEO_INFO_FAILED', message: '视频信息获取失败' };
    }
    const cid = info.data.cid;
    const title = info.data.title || '';
    const cover = info.data.pic || '';

    // 并行请求: DASH (音视频分离, 高清) 和 FLV (音视频合并, 有声音)
    const [dashResult, flvResult] = await Promise.all([
      biliGetJSON('https://api.bilibili.com/x/player/playurl', {
        bvid, cid, qn: 80, fnval: 16, fourk: 1, platform: 'web',
      }, { cookie: true, headers: { Referer: 'https://www.bilibili.com/video/' + bvid } }),
      biliGetJSON('https://api.bilibili.com/x/player/playurl', {
        bvid, cid, qn: 80, fnval: 1, platform: 'web',
      }, { cookie: true, headers: { Referer: 'https://www.bilibili.com/video/' + bvid } }),
    ]);

    // 处理 DASH 结果
    const dashOk = dashResult && dashResult.code === 0 && dashResult.data;
    const dash = dashOk ? (dashResult.data.dash || {}) : {};
    const videos = dash.video || [];
    const audios = dash.audio || [];
    const bestVideo = videos.length ? videos.reduce((a, b) => (b.bandwidth || 0) > (a.bandwidth || 0) ? b : a) : null;
    const bestAudio = audios.length ? audios.reduce((a, b) => (b.bandwidth || 0) > (a.bandwidth || 0) ? b : a) : null;

    // 处理 FLV 结果 (音视频合并, 不需要分离的音频流)
    const flvOk = flvResult && flvResult.code === 0 && flvResult.data;
    let flvUrl = '';
    let flvAudioUrl = '';  // FLV 自带音频, 不需要额外音频流
    if (flvOk && flvResult.data.durl && flvResult.data.durl.length) {
      flvUrl = flvResult.data.durl[0].url || '';
      // FLV 备用音频
      if (flvResult.data.durl[0].backup_url && flvResult.data.durl[0].backup_url.length) {
        flvAudioUrl = flvResult.data.durl[0].backup_url[0] || '';
      }
    }
    // 如果 DASH 请求也返回了 durl (FLV), 优先使用
    if (!flvUrl && dashOk && dashResult.data.durl && dashResult.data.durl.length) {
      flvUrl = dashResult.data.durl[0].url || '';
    }

    const dashVideoUrl = bestVideo ? (bestVideo.baseUrl || bestVideo.base_url || '') : '';
    const dashAudioUrl = bestAudio ? (bestAudio.baseUrl || bestAudio.base_url || '') : '';

    return {
      provider: 'bilibili',
      // url: DASH 视频流 (仅视频, 无音频)
      url: dashVideoUrl,
      // audioUrl: DASH 音频流 (仅音频)
      audioUrl: dashAudioUrl,
      // flvUrl: FLV 流 (音视频合并, 有声音!) - 客户端应优先使用
      flvUrl,
      // flvAudioUrl: FLV 备用音频 (也是合并的, 作为兜底)
      flvAudioUrl,
      bvid,
      cid,
      title,
      cover,
      // playable: 有 FLV 或 DASH 视频就认为可播
      playable: !!(flvUrl || dashVideoUrl),
      quality: (dashOk && dashResult.data.quality) || (flvOk && flvResult.data.quality) || 0,
      acceptDescription: (dashOk && dashResult.data.accept_description) || (flvOk && flvResult.data.accept_description) || [],
      acceptQuality: (dashOk && dashResult.data.accept_quality) || (flvOk && flvResult.data.accept_quality) || [],
      videoCodec: bestVideo ? bestVideo.codecs || '' : '',
      audioCodec: bestAudio ? bestAudio.codecs || '' : '',
    };
  } catch (e) {
    console.error('[BiliVideoUrl] error:', e.message);
    return { provider: 'bilibili', url: '', audioUrl: '', playable: false, error: e.message };
  }
}
async function handleBiliUserFavorites() {
  const uid = biliCookieUid();
  if (!uid) return { provider: 'bilibili', loggedIn: false, favorites: [] };
  console.log('[BiliFavorites] uid:', uid);
  try {
    const result = await biliGetJSON('https://api.bilibili.com/x/v3/fav/folder/created/list', {
      up_mid: uid,
      type: 2,
    }, { cookie: true });
    if (result && result.code === 0 && result.data && result.data.list) {
      const folders = (result.data.list || []).map(f => ({
        id: f.id || f.media_id,
        fid: f.id,
        title: f.title || f.name || '',
        cover: f.cover || '',
        mediaCount: f.media_count || f.video_count || f.count || 0,
        intro: f.intro || '',
        ctime: f.ctime || 0,
      }));
      if (folders.length) console.log('[BiliFavorites] found folders:', folders.length);
      return { provider: 'bilibili', loggedIn: true, favorites: folders };
    }
    const result2 = await biliGetJSON('https://api.bilibili.com/x/v3/fav/folder/created/list-all', {
      up_mid: uid,
      type: 2,
    }, { cookie: true });
    if (result2 && result2.code === 0 && result2.data && result2.data.list) {
      const folders = (result2.data.list || []).map(f => ({
        id: f.id || f.media_id,
        fid: f.id,
        title: f.title || f.name || '',
        cover: f.cover || '',
      ...
      if (folders.length) console.log('[BiliFavorites] found folders (list-all):', folders.length);
      return { provider: 'bilibili', loggedIn: true, favorites: folders };
    }
    return { provider: 'bilibili', loggedIn: true, favorites: [] };
  } catch (e) {
    console.error('[BiliFavorites] error:', e.message);
    return { provider: 'bilibili', loggedIn: false, favorites: [], error: e.message };
  }
}
async function handleBiliFavoriteVideos(mediaId, page, pageSize) {
  if (!mediaId) return { provider: 'bilibili', videos: [] };
  console.log('[BiliFavVideos] mediaId:', mediaId, 'page:', page);
  try {
    const result = await biliGetJSON('https://api.bilibili.com/x/v3/fav/resource/list', {
      media_id: mediaId,
      pn: page || 1,
      ps: pageSize || 20,
      type: 0,
    ...
    if (result && result.code === 0 && result.data) {
      ...
      console.log('[BiliFavVideos] got medias:', medias.length, 'has_more:', result.data.has_more);
      const videos = medias.map(m => mapBiliVideoResult({
        bvid: m.bvid || '',
        aid: m.id || m.aid,
        title: m.title || '',
        ...
        play: m.cnt_info && m.cnt_info.play || 0,
        danmaku: m.cnt_info && m.cnt_info.danmaku || 0,
        typename: '',
        pubdate: m.pubdate || m.ctime || 0,
        owner: m.upper || {},
      }));
      return {
        provider: 'bilibili',
        ...
        total: result.data.info && result.data.info.media_count || videos.length,
      };
    }
    return { provider: 'bilibili', videos: [] };
  } catch (e) {
    console.error('[BiliFavVideos] error:', e.message);
    return { provider: 'bilibili', videos: [], error: e.message };
  }
}
async function biliNormalizeCookieInput(cookieText) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'ar1s',
      productName: APP_PACKAGE.productName || 'AR1S',
      version: APP_VERSION,
      update: {
  ...
  if (pn === '/api/audio') {
    try {
      ...
      try { const aHost = new URL(audioUrl).hostname.toLowerCase(); if ((aHost.includes('bilibili.com') || aHost.includes('bilivideo.com') || aHost.includes('hdslb.com')) && biliCookie) hdr.Cookie = biliCookie; } catch (e) {}
  ...
  if (pn === '/api/bili/search') {
    try {
      ...
      const songs = await handleBiliSearch(kw, limit);
      sendJSON(res, { provider: 'bilibili', songs });
    } catch (err) {
      console.error('[BiliSearch]', err);
      sendJSON(res, { provider: 'bilibili', error: err.message, songs: [] }, 500);
    }
    return;
  }

  // ---------- Bilibili 视频播放地址 ----------
  if (pn === '/api/bili/video/url') {
    try {
      const bvid = url.searchParams.get('bvid') || '';
      const info = await handleBiliVideoUrl(bvid);
      sendJSON(res, info);
    } catch (err) {
      console.error('[BiliVideoUrl]', err);
  ...
  if (pn === '/api/bili/video/proxy') {
    try {
      ...
      while (true) { const c = await reader.read(); if (c.done) break; res.write(c.value); }
      res.end();
    } catch (err) { console.error('[BiliVideoProxy]', err); res.writeHead(500); res.end(); }
    return;
  }

  // ---------- Bilibili 登录状态 ----------
  if (pn === '/api/bili/login/status') {
    try {
      const info = await getBiliLoginInfo();
      sendJSON(res, info);
    } catch (err) {
      console.error('[BiliLoginStatus]', err);
  ...
  // ---------- Bilibili Cookie 登录 ----------
  if (pn === '/api/bili/login/cookie') {
    try {
      const body = await readRequestBody(req);
      const raw = body.cookie || body.data || body.text || '';
      const normalized = await biliNormalizeCookieInput(raw);
      const obj = parseCookieString(normalized);
      if (!obj.DedeUserID && !obj.SESSDATA) {
        sendJSON(res, { provider: 'bilibili', loggedIn: false, error: 'INVALID_BILI_COOKIE', message: 'B站 cookie 缺少 DedeUserID 或 SESSDATA' }, 400);
        return;
      }
      saveBiliCookie(normalized);
      const info = await getBiliLoginInfo();
      sendJSON(res, { ...info, saved: true });
    } catch (err) {
      console.error('[BiliLoginCookie]', err);
      sendJSON(res, { provider: 'bilibili', loggedIn: false, error: err.message }, 500);
    }
    return;
  }
  ...
  // ---------- Bilibili 登出 ----------
  if (pn === '/api/bili/logout') {
    saveBiliCookie('');
    sendJSON(res, { provider: 'bilibili', ok: true, loggedIn: false });
    return;
  }

  // ---------- Bilibili 收藏夹列表 ----------
  if (pn === '/api/bili/user/favorites') {
    try {
      const data = await handleBiliUserFavorites();
      sendJSON(res, data);
    } catch (err) {
      console.error('[BiliUserFavorites]', err);
      sendJSON(res, { provider: 'bilibili', loggedIn: false, error: err.message, favorites: [] }, 500);
    }
    return;
  }

  // ---------- Bilibili 收藏夹视频列表 ----------
  if (pn === '/api/bili/favorite/videos') {
    try {
      const mediaId = url.searchParams.get('media_id') || url.searchParams.get('id') || '';
      const page = Math.max(1, parseInt(url.searchParams.get('page') || '1', 10) || 1);
      const pageSize = Math.max(1, Math.min(50, parseInt(url.searchParams.get('page_size') || '20', 10) || 20));
      const data = await handleBiliFavoriteVideos(mediaId, page, pageSize);
      sendJSON(res, data);
    } catch (err) {
      console.error('[BiliFavVideos]', err);
      sendJSON(res, { provider: 'bilibili', error: err.message, videos: [] }, 500);
    }
    return;
  }
  ...
  // ---------- Home Hero 背景图片 ----------
  if (pn === '/api/home-hero-bg' && req.method === 'GET') {
    try {
      // 优先返回二进制图片文件 URL（v1.2.1+ 新方案）
      const imgPath = path.join(__dirname, 'public', 'assets', 'user-bg.jpg');
      if (fs.existsSync(imgPath)) {
        const stat = fs.statSync(imgPath);
        sendJSON(res, { url: '/assets/user-bg.jpg?t=' + stat.mtimeMs });
        return;
      }
      // 兼容旧版 base64 文件（逐步废弃）
      const bgPath = path.join(__dirname, '.ar1s-hero-bg');
      if (fs.existsSync(bgPath)) {
        const image = fs.readFileSync(bgPath, 'utf8');
        sendJSON(res, { image });
        return;
      } else {
        sendJSON(res, { url: '', image: '' });
        return;
      }
    } catch (e) { sendJSON(res, { image: '', error: e.message }); }
    return;
  }
  if (pn === '/api/home-hero-bg' && req.method === 'POST') {
    try {
      const contentType = (req.headers['content-type'] || '').toLowerCase();
      // 新方案：直接上传二进制文件（无大小限制，无 base64 膨胀）
      if (!contentType.includes('application/json')) {
        const chunks = [];
        for await (const chunk of req) {
          chunks.push(chunk);
          if (chunks.reduce((s, c) => s + c.length, 0) > 30 * 1024 * 1024) {
            sendJSON(res, { ok: false, error: 'FILE_TOO_LARGE' });
            return;
          }
        }
        ...
        fs.writeFileSync(path.join(imgDir, 'user-bg.jpg'), buffer);
        sendJSON(res, { ok: true, url: '/assets/user-bg.jpg' });
        return;
      }
      // 旧方案：JSON base64（兼容，逐步废弃）
      const body = await readRequestBody(req);
      const image = (body && body.image) || ''
      if (image) {
        fs.writeFileSync(path.join(__dirname, '.ar1s-hero-bg'), image, 'utf8');
        // 同时写入二进制文件以便新方案读取
        try {
          ...
          if (match) {
            const imgDir = path.join(__dirname, 'public', 'assets');
            if (!fs.existsSync(imgDir)) fs.mkdirSync(imgDir, { recursive: true });
            fs.writeFileSync(path.join(imgDir, 'user-bg.jpg'), Buffer.from(match[2], 'base64'));
          }
        } catch (e) { /* 二进制写入失败不阻塞 */ }
        sendJSON(res, { ok: true });
      } else {
        sendJSON(res, { ok: false, error: 'NO_IMAGE' });
      }
  ...
  // ---------- DIY 自定义背景媒体（持久化到磁盘，不依赖 localStorage/IndexedDB）----------
  if (pn === '/api/custom-bg' && req.method === 'GET') {
    try {
      const bgDir = path.join(__dirname, 'public', 'assets');
      const videoExts = ['.mp4', '.webm', '.mov', '.mkv'];
      let videoFile = null;
      for (const ext of videoExts) {
        const p = path.join(bgDir, 'user-custom-bg' + ext);
        if (fs.existsSync(p)) { videoFile = p; break; }
      }
      if (videoFile) {
        const stat = fs.statSync(videoFile);
        const ext = path.extname(videoFile).toLowerCase();
        sendJSON(res, { type: 'video', url: '/assets/user-custom-bg' + ext + '?t=' + stat.mtimeMs, mime: 'video/' + ext.slice(1) });
        return;
      }
      ...
      if (imgFile) {
        ...
        sendJSON(res, { type: 'image', url: '/assets/user-custom-bg' + ext + '?t=' + stat.mtimeMs });
        return;
      }
      sendJSON(res, {});
  ...
  if (pn === '/api/custom-bg' && req.method === 'POST') {
    try {
      const contentType = (req.headers['content-type'] || '').toLowerCase();
      const chunks = [];
      for await (const chunk of req) {
        chunks.push(chunk);
        if (chunks.reduce((s, c) => s + c.length, 0) > 200 * 1024 * 1024) {
          sendJSON(res, { ok: false, error: 'FILE_TOO_LARGE' });
          return;
        }
      }
      ...
      if (!isVideo && !isImage) {
        sendJSON(res, { ok: false, error: 'UNSUPPORTED_TYPE' });
        return;
      }
      const bgDir = path.join(__dirname, 'public', 'assets');
      if (!fs.existsSync(bgDir)) fs.mkdirSync(bgDir, { recursive: true });
      const oldFiles = fs.readdirSync(bgDir).filter(f => f.startsWith('user-custom-bg'));
      for (const f of oldFiles) {
        try { fs.unlinkSync(path.join(bgDir, f)); } catch (e) {}
      }
      let ext = '.jpg';
      if (isVideo) {
        const m = contentType.match(/^video\/([\w-]+)/);
        ext = m ? '.' + m[1].replace(/^quicktime$/, 'mov') : '.mp4';
      } else {
        const m = contentType.match(/^image\/([\w-]+)/);
        ext = m ? '.' + m[1].replace(/^jpeg$/, 'jpg') : '.jpg';
      }
      const filename = 'user-custom-bg' + ext;
      fs.writeFileSync(path.join(bgDir, filename), buffer);
      sendJSON(res, { ok: true, type: isVideo ? 'video' : 'image', url: '/assets/' + filename, mime: contentType });
    } catch (e) { sendJSON(res, { ok: false, error: e.message }); }
    return;
  }
  if (pn === '/api/custom-bg' && req.method === 'DELETE') {
    try {
      const bgDir = path.join(__dirname, 'public', 'assets');
