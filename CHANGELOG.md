...
- 本地扫描持久化：扫描本地音乐文件夹后自动记住文件夹与文件列表，重启应用自动恢复扫描结果，无需重新选择文件夹。
  - 桌面端改为原生文件夹对话框 + 主进程递归扫描，不再受浏览器 webkitdirectory 选择器限制。
  - 播放时按需读盘生成 Blob URL（歌曲文件在磁盘上，列表恢复只记元数据，启动更快）。
  - 修复恢复的本地歌曲加入播放队列后无法播放的问题（队列播放本地歌曲直接读盘，不再走在线音源接口）。
  - 新增"清空"按钮，可清除本次扫描结果与已保存的扫描记录。

## v1.2.2

- 重做 DIY 背景媒体持久化：走服务端磁盘存储（参考 Home Hero 背景方案），彻底摆脱 localStorage 配额和 IndexedDB 不确定性。
  - 新增 /api/custom-bg 端点（GET/POST/DELETE），图片/视频以二进制存入 public/assets/user-custom-bg.*。
  - 客户端上传改为 etch POST 二进制文件，x.backgroundMedia 只存轻量 URL（如 /assets/user-custom-bg.jpg）。
  - 启动时增加磁盘兜底：若 localStorage 引用丢失，自动从服务端文件恢复背景。
  - 
ormalizeCustomBackgroundImage / 
ormalizeCustomBackgroundMedia 接受 /assets/... 相对路径。
  - 旧 data URL / IndexedDB 格式仍兼容读取。
## v1.2.1

- 修复 B站收藏夹同步读取：增加 `list-all` 备选接口和 `type` 参数，提高 API 兼容性。
- 新增 Home 页 Hero 区域背景图片上传功能：悬停显示上传按钮，支持 JPG/PNG/WebP，自动保存并恢复。
- 歌单面板新增"本地扫描"标签：选择文件夹递归扫描 MP3/FLAC/WAV/OGG/M4A，支持直接播放和加队列。
- 移除左下角信息面板（时钟、日期、天气、颜文字、表情包上传）。
- 移除歌单面板 B站收藏夹快捷标签按钮。
- 清理无意义的仓库元数据文件。
## v1.2.0

- 新增 Bilibili (B站) 外挂登录：支持扫码/Cookie 登录，读取收藏夹，搜索 B站视频并以视频或纯音频模式播放。
- B站搜索结果展示封面、标题、UP主、播放量和时长，点击可替换背景播放视频（自动禁用粒子特效），也可仅播放音频。
- 新增"视频播放模式"开关：开启后搜索仅显示 B站结果，隐藏网易云/QQ 音乐入口和更新按钮。
- 壁纸模式完整实现：全屏壁纸窗口支持背景视频渲染、歌词居中显示、歌曲信息指示器和粒子动画，WorkerW 贴桌面层。
- Home 页升级：替换"此处施工"为三平台标识和 B站快捷搜索入口。
- 常态隐藏更新入口（桌面端标题栏）。
# Changelog

## v1.1.1

- P0 installer safety fix: installation now defaults to the first available non-C drive from `D:\AR1S` through `Z:\AR1S`; it falls back to `C:\AR1S` only when no D-Z drive exists.
- The installer now forces the target path into a dedicated `AR1S` folder, blocks non-empty non-AR1S-owned targets, and blocks C drive installs when a D-Z drive is available.
- Existing registered installs in a dedicated `...\AR1S` folder are adopted in place so users can run the new setup over their current installation and receive the safe uninstaller.
- Existing dedicated `...\AR1S` folders that already contain AR1S files can be overwritten even if they were created before the new safety marker existed.
- The new uninstaller no longer removes the whole installation root or app subfolders recursively; it deletes only known top-level AR1S/Electron files and leaves unrelated files in place.
- Legacy uninstallers without the new safety marker are skipped during install; the setup deletes only the old `Uninstall AR1S.exe` file and registry entry so it cannot indirectly trigger an older unsafe uninstall routine.
...
- 安装包支持用户自由选择安装目录；默认仍优先 `D:\AR1S`，选择盘根目录时会自动补全 `AR1S` 文件夹。
...
- 发布 AR1S 首个正式版本，GitHub 主页和安装包版本统一为 `1.0.0`。
