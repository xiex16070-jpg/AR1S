...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for AR1S icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'AR1S';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`AR1S executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`AR1S icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting AR1S resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'AR1S',
    '--set-version-string', 'ProductName', 'AR1S',
    '--set-version-string', 'CompanyName', 'AR1S',
