...
const APP_NAME = 'AR1S';
const APP_USER_MODEL_ID = 'com.ar1s.desktop';
const APP_ICON_ICO = path.join(__dirname, '..', 'build', 'icon.ico');
const NETEASE_LOGIN_PARTITION = 'persist:ar1s-netease-login';
const NETEASE_LOGIN_URL = 'https://music.163.com/#/login';
const QQ_LOGIN_PARTITION = 'persist:ar1s-qqmusic-login';
const QQ_LOGIN_URL = 'https://y.qq.com/n/ryqq/profile';
const BILIBILI_LOGIN_PARTITION = 'persist:ar1s-bilibili-login';
const BILIBILI_LOGIN_URL = 'https://www.bilibili.com/';
...
function sendGlobalHotkeyAction(action) {
  ...
  mainWindow.webContents.send('ar1s-global-hotkey', { action });
}

function unregisterAR1SGlobalHotkeys() {
...
function configureAR1SGlobalHotkeys(bindings = []) {
  unregisterAR1SGlobalHotkeys();
  const results = [];
  const seen = new Set();
...
function shouldEnsureDesktopShortcut() {
  ...
  if (process.env.AR1S_NO_DESKTOP_SHORTCUT === '1') return false;
  return app.isPackaged || process.env.AR1S_CREATE_DESKTOP_SHORTCUT === '1';
}
...
function ensureDesktopShortcut() {
  ...
  try {
    ...
    const shortcut = {
      ...
      args: '',
      description: 'AR1S desktop music player',
...
function biliCookieHasLogin(cookieText) {
  const obj = parseCookieHeader(cookieText);
  return !!(obj.DedeUserID || obj.DedeUserID__ckMd5) && !!obj.SESSDATA;
}
...
async function readBiliLoginCookieHeader(cookieSession) {
  const cookies = await cookieSession.cookies.get({});
...
async function openBilibiliLoginWindow(owner) {
  const cookieSession = session.fromPartition(BILIBILI_LOGIN_PARTITION);
  const initialCookie = await readBiliLoginCookieHeader(cookieSession);
  if (biliCookieHasLogin(initialCookie)) return { ok: true, cookie: initialCookie, reused: true };
  ...
  return new Promise((resolve) => {
    ...
    const checkCookies = async () => {
      try {
        const cookie = await readBiliLoginCookieHeader(cookieSession);
        if (biliCookieHasLogin(cookie)) {
          finish({ ok: true, cookie });
        }
      } catch (e) {
        console.warn('Bilibili login cookie check failed:', e.message);
      }
    };

    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      if (/^https?:\/\/([^/]+\.)?bilibili\.com/i.test(url)) {
        loginWindow.loadURL(url).catch((e) => console.warn('Bilibili login popup navigation failed:', e.message));
      } else if (/^https?:\/\//i.test(url)) {
        shell.openExternal(url).catch(() => {});
      }
      return { action: 'deny' };
    ...
    loginWindow.webContents.on('did-finish-load', () => {
      ...
      loginWindow.webContents.executeJavaScript(`
        setTimeout(() => {
          ...
          const loginNode = nodes.find((node) => {
            const text = (node.textContent || '').trim();
            if (!/登录/.test(text) || node.tagName === 'SPAN' && node.children.length) return false;
            const rect = node.getBoundingClientRect();
            return rect.width > 0 && rect.height > 0 && rect.width < 400;
          });
          if (loginNode) { loginNode.click(); return true; }
          return false;
        }, 1200);
    ...
    loginWindow.on('closed', async () => {
      ...
      try {
        const cookie = await readBiliLoginCookieHeader(cookieSession);
        resolve(biliCookieHasLogin(cookie)
          ? { ok: true, cookie }
          : { ok: false, cancelled: true, message: 'Bilibili 登录窗口已关闭' });
      } catch (e) {
        resolve({ ok: false, error: e.message || 'Bilibili 登录窗口已关闭' });
      }
    });

    pollTimer = setInterval(checkCookies, 1200);
    loginWindow.loadURL(BILIBILI_LOGIN_URL).catch((e) => finish({ ok: false, error: e.message }));
  });
}

async function clearBilibiliLoginSession() {
  const cookieSession = session.fromPartition(BILIBILI_LOGIN_PARTITION);
  await cookieSession.clearStorageData({
    storages: ['cookies', 'localstorage', 'indexdb', 'cachestorage'],
  });
...
public class AR1SMousePoll {
  [DllImport("user32.dll")] public static extern short GetAsyncKeyState(int vKey);
}
"@
$prev = $false
while ($true) {
  $down = (([AR1SMousePoll]::GetAsyncKeyState(4) -band 0x8000) -ne 0)
...
function broadcastDesktopLyricsLockState() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('ar1s-desktop-lyrics-lock-state', { locked });
  }
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('ar1s-desktop-lyrics-enabled-state', { enabled: !!enabled });
  }
}
...
function sendDesktopLyricsState() {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  desktopLyricsWindow.webContents.send('ar1s-desktop-lyrics-state', desktopLyricsState);
}
...
if (-not ("AR1SNativeWin" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public class AR1SNativeWin {
...
$progman = [AR1SNativeWin]::FindWindow("Progman", $null)
$result = [IntPtr]::Zero
[AR1SNativeWin]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$result) | Out-Null
$script:workerw = [IntPtr]::Zero
$enum = [AR1SNativeWin+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$param)
  $shell = [AR1SNativeWin]::FindWindowEx($top, [IntPtr]::Zero, "SHELLDLL_DefView", $null)
  if ($shell -ne [IntPtr]::Zero) {
    $script:workerw = [AR1SNativeWin]::FindWindowEx([IntPtr]::Zero, $top, "WorkerW", $null)
  }
  return $true
}
[AR1SNativeWin]::EnumWindows($enum, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { $script:workerw = $progman }
$target = [IntPtr]::new([Int64]${hwnd})
[AR1SNativeWin]::SetParent($target, $script:workerw) | Out-Null
[AR1SNativeWin]::SetWindowPos($target, [IntPtr]::Zero, 0, 0, 0, 0, 0x0013) | Out-Null
`;
...
function sendWallpaperState() {
  if (!wallpaperWindow || wallpaperWindow.isDestroyed()) return;
  wallpaperWindow.webContents.send('ar1s-wallpaper-state', wallpaperState);
}
...
ipcMain.handle('ar1s-hotkeys-configure-global', (_event, bindings) => {
  return configureAR1SGlobalHotkeys(bindings);
});

ipcMain.handle('ar1s-export-json-file', async (event, payload = {}) => {
  try {
    const owner = getSenderWindow(event);
    const defaultName = String(payload.defaultName || 'ar1s-export.json').replace(/[\\/:*?"<>|]+/g, '-');
...
ipcMain.handle('ar1s-import-json-file', async (event) => {
  try {
    const owner = getSenderWindow(event);
...
ipcMain.handle('bilibili-open-login', async (event) => {
  return openBilibiliLoginWindow(getSenderWindow(event));
});

ipcMain.handle('bilibili-clear-login', async () => {
  return clearBilibiliLoginSession();
});

ipcMain.handle('ar1s-open-update-installer', async (_event, filePath) => {
  try {
    const target = path.resolve(String(filePath || ''));
...
ipcMain.handle('ar1s-restart-app', async () => {
  try {
    app.relaunch();
    app.exit(0);
    return { ok: true };
...
ipcMain.handle('ar1s-desktop-lyrics-set-enabled', async (_event, enabled, payload) => {
  try {
    if (enabled) {
...
ipcMain.handle('ar1s-desktop-lyrics-update', async (_event, payload) => {
  try {
...
ipcMain.handle('ar1s-desktop-lyrics-set-dragging', async () => {
  return { ok: true };
});

ipcMain.handle('ar1s-desktop-lyrics-set-pointer-capture', async (_event, active) => {
  try {
...
ipcMain.handle('ar1s-desktop-lyrics-set-hot-bounds', async (_event, bounds) => {
  try {
...
ipcMain.handle('ar1s-desktop-lyrics-set-lock-state', async (_event, locked) => {
  try {
...
ipcMain.handle('ar1s-desktop-lyrics-move-by', async (_event, dx, dy) => {
  try {
