...
```powershell
cd E:\桌面\播放器软件\AR1S\resources\app
...
- GitHub 仓库已公开：`https://github.com/XxHuberrr/AR1S`
- `v1.1.0` Release：`https://github.com/XxHuberrr/AR1S/releases/tag/v1.1.0`
...
- 没有本地 `ar1s-lyric-layout-v1` 时，`readSavedLyricLayout()` 使用 packaged 默认快照；没有本地用户存档 key 时自动创建「默认测试」槽位。
...
- 已生成安装包：`dist/AR1S-1.1.0-Setup.exe`。
- 已生成校验文件：`dist/AR1S-1.1.0-SHA256SUMS.txt`。
...
- Release 建议上传：
  - `dist/AR1S-1.1.0-Setup.exe`
  - `dist/AR1S-1.1.0-Setup.exe.blockmap`
  - `dist/AR1S-1.1.0-SHA256SUMS.txt`
